...
type MetricsCollector struct {
	...
	workflowEnrichmentsTotal      *prometheus.HistogramVec
	workflowEnrichmentErrorsTotal *prometheus.CounterVec
...
func NewMetricsCollector(log logger_interfaces.LoggerInterface) interfaces.MetricsInterface {
	...
	mc.workflowsExecutedTotal = mc.registerCollector(prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cano_workflows_executed_total",
			Help: "Total number of workflows executed",
		},
		[]string{"workflow_name", "status"},
	), "workflowsExecutedTotal").(*prometheus.CounterVec)

	mc.workflowEnrichmentsTotal = mc.registerCollector(prometheus.NewHistogramVec(
		prometheus.HistogramOpts{
			Name:    "cano_workflow_enrichments_total",
			Help:    "Number of enrichments generated by workflows",
			Buckets: []float64{0, 1, 2, 5, 10, 20, 50, 100},
		},
		[]string{"workflow_name"},
	), "workflowEnrichmentsTotal").(*prometheus.HistogramVec)

	mc.workflowEnrichmentErrorsTotal = mc.registerCollector(prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: "cano_workflow_enrichment_errors_total",
			Help: "Total number of workflow enrichment errors",
		},
		[]string{"workflow_name", "error_type"},
	), "workflowEnrichmentErrorsTotal").(*prometheus.CounterVec)
...
// Workflow metrics implementations
func (mc *MetricsCollector) IncWorkflowsExecuted(workflowName, status string) {
	mc.workflowsExecutedTotal.WithLabelValues(workflowName, status).Inc()
	mc.logger.Debugf("Incremented workflows executed counter for workflow: %s, status: %s", workflowName, status)
}

func (mc *MetricsCollector) ObserveWorkflowEnrichments(workflowName string, enrichmentCount int) {
	mc.workflowEnrichmentsTotal.WithLabelValues(workflowName).Observe(float64(enrichmentCount))
	mc.logger.Debugf("Observed workflow enrichments for workflow: %s, count: %d", workflowName, enrichmentCount)
}

func (mc *MetricsCollector) IncWorkflowEnrichmentErrors(workflowName, errorType string) {
	mc.workflowEnrichmentErrorsTotal.WithLabelValues(workflowName, errorType).Inc()
	mc.logger.Debugf("Incremented workflow enrichment errors counter for workflow: %s, error_type: %s", workflowName, errorType)
}
