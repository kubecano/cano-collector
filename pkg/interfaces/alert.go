...
type AlertDispatcherInterface interface {
	DispatchAlert(ctx context.Context, alert *alert.AlertManagerEvent, team *config_team.Team) error
}
...
type AlertFormatterInterface interface {
	FormatAlert(alert *alert.AlertManagerEvent) string
}
...
type TeamResolverInterface interface {
	ResolveTeam(alert *alert.AlertManagerEvent) (*config_team.Team, error)
