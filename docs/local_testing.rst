...
This guide explains how to test **cano-collector** locally on **macOS** using **k3d** (lightweight Kubernetes in Docker).
...
Edit ``values-local.yaml`` in the project root:
    ...
    destinations:
      slack:
        - name: "dev-local-slack"
          api_key: "xoxb-YOUR-DEV-TOKEN-HERE"  # ← Replace
          slack_channel: "#your-dev-channel"   # ← Replace
...
.. note::
    ...
    **Port 5001** is used instead of 5000 to avoid conflict with macOS AirPlay Receiver (Control Center).
...
.. note::
    ...
    For real alerts, you need **Prometheus** + **Alertmanager** running in the cluster. Test pods trigger alerts via kube-state-metrics metrics.
...
.. list-table::
   ...
   * - Command
     ...
     - When to Use
   * - ``make local-dev``
     - Full setup (tests + build + deploy)
     - First time, major changes
   * - ``make local-dev-quick``
     - Rebuild + redeploy (skip tests)
     - Code changes
   * - ``make local-dev-config``
     - Config-only update (no rebuild)
     - values-local.yaml changes
   * - ``make local-logs``
     - Tail logs
     - Debugging
   * - ``make local-status``
...
* **Local registry** - Images built and pushed locally (port 5001)
...
* **``values-local.yaml``** - Local Helm values (Slack config, workflows)
  ...
  * Compared with production: ArgoCD repo at ``apps/kubecano/cano-collector/values.yaml``
...
* :doc:`development_guide` - Project architecture and development practices
* ``tests/README.md`` - Test pod scenarios
* `k3d Documentation <https://k3d.io/>`_ - k3d reference
