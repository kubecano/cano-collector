...
This decoupled architecture allows for great flexibility. For instance, to support a new notification service, one only needs to implement a new Sender and the appropriate Destination configuration, without changing the logic for generating or enriching issues. 
